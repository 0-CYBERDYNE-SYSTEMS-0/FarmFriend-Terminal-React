  ...
  tell application "Contacts"
    try
      ...
      if emailAddress is "" or emailAddress is missing value then
        set emailAddress to "{emailAddress}"
      end if
      ...
      if emailAddress is not "" and emailAddress is not "{emailAddress}" then
        make new email at end of emails of newPerson with properties {label:"work", value:emailAddress}
      end if
      
      -- Add phone if provided
      if phoneNumber is not "" and phoneNumber is not "{phoneNumber}" then
