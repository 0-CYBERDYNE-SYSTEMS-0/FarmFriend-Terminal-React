...
on run
    ...
    if "{action}" is not equal to "{action}" then
        set actionToUse to "{action}"
    end if
    
    if "{automationName}" is not equal to "{automationName}" then
        set automationNameToUse to "{automationName}"
    end if
