  ...
  tell application "Mail"
    try
      ...
      if ruleName is "" or ruleName is missing value then
        set ruleName to "{ruleName}"
      end if
      
      if searchField is "" or searchField is missing value then
        set searchField to "{searchField}" -- e.g., "From", "Subject", "To", "Entire Message"
      end if
      
      if searchText is "" or searchText is missing value then
        set searchText to "{searchText}"
      end if
      
      if actionType is "" or actionType is missing value then
        set actionType to "{actionType}" -- e.g., "Move Message", "Copy Message", "Set Color", "Mark as Read"
      end if
      
      if actionTarget is "" or actionTarget is missing value then
        set actionTarget to "{actionTarget}" -- e.g., mailbox name for move/copy, or color name
      end if
