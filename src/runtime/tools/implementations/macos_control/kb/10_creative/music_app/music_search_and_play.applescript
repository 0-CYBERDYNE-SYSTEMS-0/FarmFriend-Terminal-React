...
set searchTermParam to "{search_term}"
if searchTermParam is "" or searchTermParam is "{search_term}" then
  return "Error: No search term provided. Please specify a search_term parameter."
end if

-- Get search type parameter
set searchTypeParam to "{search_type}"
if searchTypeParam is "" or searchTypeParam is "{search_type}" then
  set searchTypeParam to "songs" -- Default search type: songs
end if

-- Get play results parameter
set playResultsParam to "{play_results}"
if playResultsParam is "" or playResultsParam is "{play_results}" then
