...
set actionParam to "{action}"
if actionParam is "" or actionParam is "{action}" then
  set actionParam to "stats" -- Default action: show library stats
end if

-- Get rating parameter (for rate_current action)
set ratingParam to "{rating}"
if ratingParam is "" or ratingParam is "{rating}" then
  set ratingParam to "5" -- Default rating: 5 stars
end if

-- Get genre parameter (for update_genre action)
set genreParam to "{genre}"
if genreParam is "" or genreParam is "{genre}" then
  set genreParam to "" -- Will be validated later if needed
end if

-- Get file path parameter (for add_file action)
set filePathParam to "{file_path}"
if filePathParam is "" or filePathParam is "{file_path}" then
