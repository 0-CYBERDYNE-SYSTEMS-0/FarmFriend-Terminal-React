...
set mediaPathParam to "{media_path}"
if mediaPathParam is "" or mediaPathParam is "{media_path}" then
  return "Error: No media path or URL provided. Please specify a file path or streaming URL."
end if

set startTimeParam to "{start_time}"
if startTimeParam is "" or startTimeParam is "{start_time}" then
  set startTimeParam to "0" -- Default: start at beginning
end if

set fullscreenParam to "{fullscreen}"
if fullscreenParam is "" or fullscreenParam is "{fullscreen}" then
