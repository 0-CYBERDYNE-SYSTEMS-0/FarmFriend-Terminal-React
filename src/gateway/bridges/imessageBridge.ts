...
import { getScript, substituteParams, validateParams } from "../../runtime/tools/implementations/macos_control/scriptLoader.js";
...
function getKBDir(): string {
  ...
  return join(currentDir, "..", "..", "runtime", "tools", "implementations", "macos_control", "kb");
}
...
function normalizeWatchList(raw?: IMessageBridgeConfig["watchChats"]): WatchTarget[] {
...
export class IMessageBridge implements GatewayBridge {
  name = "imessage";
  ...
  async start(): Promise<void> {
    ...
    if (!isMac()) {
      this.lastError = "iMessage bridge requires macOS";
      return;
    }
